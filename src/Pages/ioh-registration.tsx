...
export default function IOHRegistration() {
  ...
  // We'll simulate some slots being partially filled or full for demonstration
  const slotCapacities = {
    schoolCollege: {
      '2jan_forenoon': { max: 6500, current: 0 },
      '2jan_afternoon': { max: 4000, current: 0 }, // This one is full
      '3jan_forenoon': { max: 5500, current: 0 },
      '3jan_afternoon': { max: 3000, current: 0 }
    },
    overall: {
      '2jan_forenoon': { max: 13500, current: 0 },
      '2jan_afternoon': { max: 16000, current: 0 }, // This one is full
  ...
  // const aadharNeeded = Math.ceil(totalPeople / 4); // Logic removed as per previous file
  ...
  const handleSubmit = (e) => {
    ...
    // And also update the slotCapacities.overall[selectedSlot].current
  };
  ...
  const getAvailableSlots = () => {
    ...
    if (isSchoolCollege) {
      return [
        ...
        { value: '3jjan_afternoon', label: '3rd Jan - Afternoon', capacity: slotCapacities.schoolCollege['3jan_afternoon'] }
      ];
    }
        ...
        <div className="max-w-4xl mx-auto bg-gray-900 text-gray-200 rounded-2xl shadow-2xl shadow-yellow-500/10 p-6 md:p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="bg-gray-800 p-6 rounded-lg">
              ...
              <div className="space-y-3">
                {getAvailableSlots().map((slot) => {
                  ...
                  // const remaining = slot.capacity.max - slot.capacity.current; // No longer needed for display
                  ...
                  return (
                    <label
                      ...
                      className={`block p-4 border-2 rounded-lg transition-all ${
                        isFull
                          ? 'border-gray-700 bg-gray-800 cursor-not-allowed opacity-60' // Added opacity for clarity
                    ...
                    >
                      <input
                        ...
                        disabled={isFull}
                        required
                        className="mr-3 text-yellow-500 focus:ring-yellow-600" // Styled radio button
                      />
                      <span className={`font-medium ${isFull ? 'text-gray-500' : 'text-gray-100'}`}>{slot.label}</span>
                      
                      {/* --- THIS IS THE MODIFIED BLOCK --- */}
                      <span className={`ml-3 text-sm ${isFull ? 'text-red-500 font-semibold' : 'text-green-400'}`}>
                        {isFull 
                          ? '(FULL)' 
                          : (slot.label.includes('Forenoon') ? '(Starts at 9 AM)' : '(Starts at 1 PM)')
                        }
                      </span>
            ...
            <div className="text-center pt-6">
              <button
                type="submit"
                className="bg-yellow-500 text-black px-12 py-4 rounded-lg font-semibold text-lg hover:bg-yellow-600 transition-colors shadow-lg shadow-yellow-500/30 w-full sm:w-auto hover:cursor-pointer disabled:bg-gray-500 disabled:shadow-none disabled:cursor-not-allowed"
                // Simple validation: disable if no slot is selected
